...
esp_err_t hermes_display_init(void)
{
    ...
    /* Switch touch input to polling mode. The BSP registers ST7123 touch in
     * interrupt mode (GPIO 23), but the interrupt may not fire reliably.
     * Polling mode ensures LVGL reads touch data on every timer tick. */
